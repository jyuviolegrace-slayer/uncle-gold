...
export class NineSlice {
    ...
    private createNineSliceTextures(
        ...
        if (texture.key === "__MISSING") {
            console.warn(
                `[NineSlice:${methodName}] the provided texture asset key was not found: ${assetKey}`
            );
            return;
        }
        ...
        if (!frames["__BASE"]) {
            console.warn(
                `[NineSlice:${methodName}] the provided texture asset key does not have a base texture: ${assetKey}`
            );
            return;
        }
        ...
        if (texture.getFrameNames(false).length !== 0) {
            console.debug(
                `[NineSlice:${methodName}] the provided texture asset key already has additional frames: ${assetKey}`
            );
            return;
        }
    ...
    createNineSliceContainer(
        ...
        const tl = scene.add
            .image(0, 0, assetKey, AssetCutFrame.TL)
            .setOrigin(0);
        tl.setData(ASSET_CUT_FRAME_DATA_MANAGER_NAME, AssetCutFrame.TL);

        const tm = scene.add
            .image(tl.displayWidth, 0, assetKey, AssetCutFrame.TM)
            .setOrigin(0);
        ...
        const bm = scene.add
            .image(bl.displayWidth, bl.y, assetKey, AssetCutFrame.BM)
            .setOrigin(0);
