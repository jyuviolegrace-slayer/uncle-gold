...
export class DataManager {
  ...
  loadData(): void {
    if (typeof Storage === 'undefined') {
      console.warn('[DataManager:loadData] localStorage is not supported, will not be able to save and load data.');
      return;
    }
    ...
    try {
      ...
      console.warn('[DataManager:loadData] encountered an error while attempting to load and parse saved data.', error);
    }
  }

  saveData(): void {
    if (typeof Storage === 'undefined') {
      console.warn('[DataManager:saveData] localStorage is not supported, will not be able to save and load data.');
      return;
    }
  ...
  getAnimatedTextSpeed(): number {
    ...
    const chosenTextSpeed = this.store.get(DataManagerStoreKeys.OPTIONS_TEXT_SPEED) as TextSpeedOptions;
    return TEXT_SPEEDS[chosenTextSpeed] || TEXT_SPEEDS[TextSpeedOptions.MID];
  }

  getInventory(): InventoryItem[] {
    const items: InventoryItem[] = [];
    const inventory = this.store.get(DataManagerStoreKeys.INVENTORY) as LegacyInventoryItem[];
  ...
  addItemPickedUp(itemId: number): void {
    const itemsPickedUp = this.store.get(DataManagerStoreKeys.ITEMS_PICKED_UP) as number[] || [];
    itemsPickedUp.push(itemId);
    this.store.set(DataManagerStoreKeys.ITEMS_PICKED_UP, itemsPickedUp);
  }

  isPartyFull(): boolean {
    const partySize = this.store.get(DataManagerStoreKeys.CRITTERS_IN_PARTY) as CritterInstance[] || [];
    return partySize.length >= 6;
  }

  getParty(): CritterInstance[] {
    return this.store.get(DataManagerStoreKeys.CRITTERS_IN_PARTY) as CritterInstance[] || [];
  }
  ...
  viewedEvent(eventId: number): void {
    const viewedEvents = new Set(this.store.get(DataManagerStoreKeys.VIEWED_EVENTS) as number[] || []);
    viewedEvents.add(eventId);
    this.store.set(DataManagerStoreKeys.VIEWED_EVENTS, Array.from(viewedEvents));
  }

  getFlags(): Set<GameFlag> {
    return new Set(this.store.get(DataManagerStoreKeys.FLAGS) as GameFlag[] || []);
  }

  addFlag(flag: GameFlag): void {
    const existingFlags = new Set(this.store.get(DataManagerStoreKeys.FLAGS) as GameFlag[] || []);
  ...
  private updateDataManager(data: GlobalState): void {
    this.store.set({
      ...
      [DataManagerStoreKeys.PLAYER_LOCATION]: data.player.location || { ...initialState.player.location },
      [DataManagerStoreKeys.OPTIONS_TEXT_SPEED]: data.options.textSpeed,
      [DataManagerStoreKeys.OPTIONS_BATTLE_SCENE_ANIMATIONS]: data.options.battleSceneAnimations,
      [DataManagerStoreKeys.OPTIONS_BATTLE_STYLE]: data.options.battleStyle,
      ...
      [DataManagerStoreKeys.ITEMS_PICKED_UP]: data.itemsPickedUp || [...initialState.itemsPickedUp],
      [DataManagerStoreKeys.VIEWED_EVENTS]: data.viewedEvents || [...initialState.viewedEvents],
  ...
  private dataManagerDataToGlobalStateObject(): GlobalState {
    return {
      player: {
        position: {
          x: (this.store.get(DataManagerStoreKeys.PLAYER_POSITION) as PlayerPosition)?.x || 0,
          y: (this.store.get(DataManagerStoreKeys.PLAYER_POSITION) as PlayerPosition)?.y || 0,
        },
        direction: this.store.get(DataManagerStoreKeys.PLAYER_DIRECTION) as Direction || Direction.DOWN,
        location: { ...(this.store.get(DataManagerStoreKeys.PLAYER_LOCATION) as PlayerLocation) },
      },
      options: {
        textSpeed: this.store.get(DataManagerStoreKeys.OPTIONS_TEXT_SPEED) as TextSpeedOptions || TextSpeedOptions.MID,
        battleSceneAnimations: this.store.get(DataManagerStoreKeys.OPTIONS_BATTLE_SCENE_ANIMATIONS) as BattleSceneOptions || BattleSceneOptions.ON,
        battleStyle: this.store.get(DataManagerStoreKeys.OPTIONS_BATTLE_STYLE) as BattleStyleOptions || BattleStyleOptions.SHIFT,
        sound: this.store.get(DataManagerStoreKeys.OPTIONS_SOUND) as SoundOptions || SoundOptions.ON,
        volume: this.store.get(DataManagerStoreKeys.OPTIONS_VOLUME) as number || 4,
        menuColor: this.store.get(DataManagerStoreKeys.OPTIONS_MENU_COLOR) as number || 0,
      },
      gameStarted: this.store.get(DataManagerStoreKeys.GAME_STARTED) as boolean || false,
      monsters: {
        inParty: [...(this.store.get(DataManagerStoreKeys.CRITTERS_IN_PARTY) as CritterInstance[] || [])],
      },
      inventory: this.store.get(DataManagerStoreKeys.INVENTORY) as LegacyInventoryItem[] || [],
      itemsPickedUp: [...(this.store.get(DataManagerStoreKeys.ITEMS_PICKED_UP) as number[] || [])],
      viewedEvents: [...(this.store.get(DataManagerStoreKeys.VIEWED_EVENTS) as number[] || [])],
      flags: [...(this.store.get(DataManagerStoreKeys.FLAGS) as GameFlag[] || [])],
